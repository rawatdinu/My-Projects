...
Public Class frmCustomerAccount
    ...
    Private Sub cmdFindItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdFindItem.Click
        ...
        Try
            StrQuery = "SELECT * from CustomerMaster1 order by CustomerName"
            ...
            If DS_Search.Tables("FillGrid").Rows.Count > 0 Then
                With dgSearch
                    For i = 0 To DS_Search.Tables("FillGrid").Rows.Count - 1
                        ...
                        .Rows(i).Cells(1).Value = IIf(IsDBNull(DS_Search.Tables("FillGrid").Rows(i).Item("CustomerCode")), "", DS_Search.Tables("FillGrid").Rows(i).Item("CustomerCode"))
                        .Rows(i).Cells(2).Value = IIf(IsDBNull(DS_Search.Tables("FillGrid").Rows(i).Item("CustomerName")), "", DS_Search.Tables("FillGrid").Rows(i).Item("CustomerName"))
                        ...
                        '.Rows(i).Cells(4).Value = IIf(IsDBNull(ds.Tables("FillGrid").Rows(i).Item("City")), "", ds.Tables("FillGrid").Rows(i).Item("City"))
                        '.Rows(i).Cells(5).Value = IIf(IsDBNull(ds.Tables("FillGrid").Rows(i).Item("State")), "", ds.Tables("FillGrid").Rows(i).Item("State"))
    ...
    Private Sub FillGridInfo()
        ...
        With dgDetail
            ...
            .Columns(4).Width = 80
            .Columns(5).Name = "SaleAmount"
            .Columns(5).Width = 80
            .Columns(6).Name = "RecvAmount"
            .Columns(6).Width = 80
        ...
        '    .set_TextMatrix(0, 5, "SaleAmount")
        '    .set_ColWidth(5, 1300)

        '    .set_TextMatrix(0, 6, "RecvAmount")
        '    .set_ColWidth(6, 1300)
    ...
    Private Sub FillSearchGridInfo()
        ...
        '    .set_TextMatrix(0, 1, "CustomerCode")
        '    .set_ColWidth(1, 2000)
    ...
    Private Sub FillGridItem()
        ...
        '            .set_TextMatrix(.Rows - 1, 1, IIf(IsDBNull(drDisplay.Item("CustomerCode")), "", drDisplay.Item("CustomerCode")))
        '            .set_TextMatrix(.Rows - 1, 2, IIf(IsDBNull(drDisplay.Item("CustomerName")), "", drDisplay.Item("CustomerName")))
    ...
    Private Sub frmCustomerAccount_Disposed(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Disposed
        CustomerAcc = Nothing
    End Sub
    ...
    Private Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button2.Click
        ...
        '''*********************************************OpeningBalance**************************************************
        Strquery = "SELECT OpeningBalance As  Debit FROM CustomerMaster1 where Customercode='" & txtCustomerCode.Text & "'  "

        da = New SqlClient.SqlDataAdapter(Strquery, gl_Con)
        da.Fill(ds, "MR")

        If ds.Tables("MR").Rows.Count > 0 Then
            OpDebit = IIf(IsDBNull(ds.Tables("MR").Rows(0).Item("Debit")), 0, ds.Tables("MR").Rows(0).Item("Debit"))
        Else
            OpDebit = 0
            OpCredit = 0
        End If
        ...
        Strquery = "SELECT Sum(Ledger.Debit) As Debit,Sum( Ledger.Credit) As Credit FROM Ledger where LedgerCode='" & txtCustomerCode.Text & "' and Ledger.TransactionDate < '" & convertToServerDateFormat(dtpFromDate.Value) & "' "
        ...
        Strquery = "SELECT Ledger.TransactionNo, Ledger.TransactionDate, Ledger.LedgerCode, Ledger.TransactionType, Ledger.Debit, Ledger.Credit, Ledger.Narration FROM Ledger   WHERE  Ledger.LedgerCode='" & txtCustomerCode.Text & "'And Ledger.TransactionDate >= '" & convertToServerDateFormat(dtpFromDate.Value) & "'  and Ledger.TransactionDate <= '" & convertToServerDateFormat(dtpToDate.Value) & "' "
        ...
        '        Strquery = "Insert into Temp_PartyAccoumt(Sno,FromDate,Todate,TransactionNo,TransactionDate,ItemCode,Issued, Issued1, Rate,SubRate,InvoiceNo,ChallanNo,PartyName,Balance,Balance1) values(2,'" & dtpFromDate.Value & "','" & dtpToDate.Value & "','" & TrnNo & "','" & TrnDate & "','" & txtCustomerCode.Text & "'," & qty & "," & subqty & "," & rate & "," & SubRate & ",'" & InvoiceNo & "','" & ChallanNo & "','" & PartyName & "'," & Opqty & "," & Opsubqty & " )"
            ...
            With dgDetail
                For i = 0 To ds.Tables("MR").Rows.Count - 1
                    ...
                    If Trim(.Rows(i).Cells(1).Value.ToString) = "Opening" Then
                        txtOpening.Text = Val(.Rows(i).Cells(5).Value) - Val(.Rows(i).Cells(6).Value)
                    End If
        ...
        '            txtOpening.Text = Val(.get_TextMatrix(i + 1, 5)) - Val(.get_TextMatrix(i + 1, 6))
        '        End If
    ...
    Private Sub cmdPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPrint.Click
        ...
        fSalesReportViewer.Text = "Customer Account Statement"
        ...
        Try
            ...
            With dgDetail
                For i = 0 To .RowCount - 1
                    Strquery = "Insert into Temp_PartyAccoumt(Sno,FromDate,Todate,TransactionNo,TransactionDate,LedgerCode,Debit,TransactionType,Credit,Balance,DBCR, Narration,InvoiceNo,ChallanNo ) values (" & Val(.Rows(i).Cells(0).Value) & ",'" & convertToServerDateFormat(dtpFromDate.Value) & "','" & convertToServerDateFormat(dtpToDate.Value) & "','" & (.Rows(i).Cells(1).Value) & "','" & (.Rows(i).Cells(2).Value) & "','" & txtCustomerCode.Text & "','" & Val(.Rows(i).Cells(5).Value) & "','" & (.Rows(i).Cells(3).Value) & "','" & Val(.Rows(i).Cells(6).Value) & "','" & Val(.Rows(i).Cells(8).Value) & "','" & (.Rows(i).Cells(9).Value) & "','" & (.Rows(i).Cells(12).Value) & "','" & (.Rows(i).Cells(10).Value) & "','" & (.Rows(i).Cells(11).Value) & "')"
            ...
            CrRepDoc.Load(Application.StartupPath & "\Report\rptCustomerAccountStatement.rpt")
    ...
    Private Sub txtSearch_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtSearch.TextChanged
        ...
        Try
            ...
            dv = New DataView(DS_Search.Tables(0), "CustomerName Like '" & Trim(txtSearch.Text) & "*" & "'", "CustomerCode", DataViewRowState.CurrentRows)
            dTable = dv.ToTable
            dgSearch.RowCount = 1
        ...
        'Dim drDisplay As DataRow
        'fgSearch.Rows = 1
        'Strquery = "Select CustomerCode,dTable,Address,Balance from CustomerMaster1 WHERE     (dTable  LIKE '%" & txtSearch.Text & "%')  order by Customername"
        'f_daDept = New SqlClient.SqlDataAdapter(Strquery, gl_Con)
        'f_dsDept.Clear()
        'f_daDept.Fill(f_dsDept, "ItemList")
        'For iRow = 0 To f_dsDept.Tables("ItemList").Rows.Count - 1
    ...
    Private Sub fgDetail_DblClick(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        'If fgDetail.get_TextMatrix(fgDetail.RowSel, 3) = "SaleInvoice" Then
    ...
    Private Sub dgDetail_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgDetail.DoubleClick
        If dgDetail.RowCount >= 1 Then
            If dgDetail(3, Convert.ToInt32(dgDetail.CurrentRow.Index)).Value.ToString = "SaleInvoice" Then
